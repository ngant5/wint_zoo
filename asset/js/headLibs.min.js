var WRS;
(function(){if(!WRS||!WRS.requirejs){var requirejs,require,define;
if(!WRS){WRS={}
}else{require=WRS
}window.cookie=(function(){var obj={};
var settings={name:"WRS",value:"TRUE",hourstolive:24*365};
obj.get=function(name){var v=document.cookie.match("(^|;) ?"+name+"=([^;]*)(;|$)");
return v?v[2]:null
};
obj.set=function(name,value,hourstolive){var hours=+hourstolive;
var date=new Date();
date.setTime(date.getTime()+(hours||1)*60*60*1000);
document.cookie=name+"="+value+";path=/;expires="+date.toGMTString()
};
obj.del=function(name,confirm){if(typeof confirm!=="string"){return'This feature is for debugging purposes only. If you know what you are doing, please add a 2nd parameter to acknowledge as follows: del("mycookiename", "CONFIRM DELETE")'
}if(confirm!=="CONFIRM DELETE"){return'Please acknowledge by using "CONFIRM DELETE".'
}var hours=settings.hourstolive;
var date=new Date();
date.setTime(date.getTime()-(hours||1)*60*60*1000);
document.cookie=name+"=;path=/;expires="+date.toGMTString()
};
obj.allowed=function(){obj.set("TWRS","true",0.1);
if(obj.get("TWRS")!=null){obj.del("TWRS","CONFIRM DELETE");
return true
}return false
};
return obj
...
}if(name){if(prefix){if(isNormalized){normalizedName=name
}else{if(pluginModule&&pluginModule.normalize){normalizedName=pluginModule.normalize(name,function(name){return normalize(name,parentName,applyMap)
})
...
if(typeof WRS.define==="function"&&WRS.define.amd){WRS.define("jquery",[],function(){return jQuery
})
 ...
 */
;
window.Detectizr=(function(i,a,o,e){var j={},h=i.Modernizr,A=["tv","tablet","mobile","desktop"],b={addAllFeaturesAsClass:false,detectDevice:true,detectDeviceModel:true,detectScreen:true,detectOS:true,detectBrowser:true,detectPlugins:true},d=[{name:"adobereader",substrs:["Adobe","Acrobat"],progIds:["AcroPDF.PDF","PDF.PDFCtrl.5"]},{name:"flash",substrs:["Shockwave Flash"],progIds:["ShockwaveFlash.ShockwaveFlash.1"]},{name:"wmplayer",substrs:["Windows Media"],progIds:["wmplayer.ocx"]},{name:"silverlight",substrs:["Silverlight"],progIds:["AgControl.AgControl"]},{name:"quicktime",substrs:["QuickTime"],progIds:["QuickTime.QuickTime"]}],k=/[\t\r\n]/g,z=o.documentElement,w,x;
function v(F,D){var C,B,E;
if(arguments.length>2){for(C=1,B=arguments.length;
C<B;
C+=1){v(F,arguments[C])
}}else{for(E in D){if(D.hasOwnProperty(E)){F[E]=D[E]
}}}return F
}function g(B){return j.browser.userAgent.indexOf(B)>-1
}function t(B){return B.test(j.browser.userAgent)
}function p(B){return B.exec(j.browser.userAgent)
}function r(B){return B.replace(/^\s+|\s+$/g,"")
}function f(B){if(B===null||B===e){return""
}return String(B).replace(/((\s|\-|\.)+[a-z0-9])/g,function(C){return C.toUpperCase().replace(/(\s|\-|\.)/g,"")
})
}function n(B,C){var D=C||"",E=B.nodeType===1&&(B.className?(" "+B.className+" ").replace(k," "):"");
if(E){while(E.indexOf(" "+D+" ")>=0){E=E.replace(" "+D+" "," ")
}B.className=C?r(E):""
}}function c(B,C,D){if(!!B){B=f(B);
if(!!C){C=f(C);
m(B+C,true);
if(!!D){m(B+C+"_"+D,true)
}}}}function m(B,C){if(!!B&&!!h){if(b.addAllFeaturesAsClass){h.addTest(B,C)
}else{C=typeof C==="function"?C():C;
if(C){h.addTest(B,true)
}else{delete h[B];
n(z,B)
}}}}function u(C,B){C.version=B;
var D=B.split(".");
if(D.length>0){D=D.reverse();
C.major=D.pop();
if(D.length>0){C.minor=D.pop();
if(D.length>0){D=D.reverse();
C.patch=D.join(".")
}else{C.patch="0"
}}else{C.minor="0"
}}else{C.major="0"
}}function s(){i.clearTimeout(w);
w=i.setTimeout(function(){x=j.device.orientation;
if(i.innerHeight>i.innerWidth){j.device.orientation="portrait"
}else{j.device.orientation="landscape"
}m(j.device.orientation,true);
if(x!==j.device.orientation){m(x,false)
}},10)
}function q(F){var C=a.plugins,G,H,B,E,D;
for(E=C.length-1;
E>=0;
E--){G=C[E];
H=G.name+G.description;
B=0;
for(D=F.length;
D>=0;
D--){if(H.indexOf(F[D])!==-1){B+=1
}}if(B===F.length){return true
}}return false
}function l(B){var C;
for(C=B.length-1;
C>=0;
C--){try{new ActiveXObject(B[C])
}catch(D){}}return false
}function y(E){var D,C,G,I,B,H,F;
b=v({},b,E||{});
if(b.detectDevice){j.device={type:"",model:"",orientation:""};
G=j.device;
if(t(/googletv|smarttv|smart-tv|internet.tv|netcast|nettv|appletv|boxee|kylo|roku|dlnadoc|roku|pov_tv|hbbtv|ce\-html/)){G.type=A[0];
G.model="smartTv"
}else{if(t(/xbox|playstation.3|wii/)){G.type=A[0];
G.model="gameConsole"
}else{if(t(/ip(a|ro)d/)){G.type=A[1];
G.model="ipad"
}else{if((t(/tablet/)&&!t(/rx-34/))||t(/folio/)){G.type=A[1];
G.model=String(p(/playbook/)||"")
}else{if(t(/linux/)&&t(/android/)&&!t(/fennec|mobi|htc.magic|htcX06ht|nexus.one|sc-02b|fone.945/)){G.type=A[1];
G.model="android"
}else{if(t(/kindle/)||(t(/mac.os/)&&t(/silk/))){G.type=A[1];
G.model="kindle"
}else{if(t(/gt-p10|sc-01c|shw-m180s|sgh-t849|sch-i800|shw-m180l|sph-p100|sgh-i987|zt180|htc(.flyer|\_flyer)|sprint.atp51|viewpad7|pandigital(sprnova|nova)|ideos.s7|dell.streak.7|advent.vega|a101it|a70bht|mid7015|next2|nook/)||(t(/mb511/)&&t(/rutem/))){G.type=A[1];
G.model="android"
}else{if(t(/bb10/)){G.type=A[1];
G.model="blackberry"
}else{G.model=p(/iphone|ipod|android|blackberry|opera mini|opera mobi|skyfire|maemo|windows phone|palm|iemobile|symbian|symbianos|fennec|j2me/);
if(G.model!==null){G.type=A[2];
G.model=String(G.model)
}else{G.model="";
if(t(/bolt|fennec|iris|maemo|minimo|mobi|mowser|netfront|novarra|prism|rx-34|skyfire|tear|xv6875|xv6975|google.wireless.transcoder/)){G.type=A[2]
}else{if(t(/opera/)&&t(/windows.nt.5/)&&t(/htc|xda|mini|vario|samsung\-gt\-i8000|samsung\-sgh\-i9/)){G.type=A[2]
}else{if((t(/windows.(nt|xp|me|9)/)&&!t(/phone/))||t(/win(9|.9|nt)/)||t(/\(windows 8\)/)){G.type=A[3]
}else{if(t(/macintosh|powerpc/)&&!t(/silk/)){G.type=A[3];
G.model="mac"
}else{if(t(/linux/)&&t(/x11/)){G.type=A[3]
}else{if(t(/solaris|sunos|bsd/)){G.type=A[3]
}else{if(t(/cros/)){G.type=A[3]
}else{if(t(/bot|crawler|spider|yahoo|ia_archiver|covario-ids|findlinks|dataparksearch|larbin|mediapartners-google|ng-search|snappy|teoma|jeeves|tineye/)&&!t(/mobile/)){G.type=A[3];
G.model="crawler"
}else{G.type=A[2]
}}}}}}}}}}}}}}}}}for(D=0,C=A.length;
D<C;
D+=1){m(A[D],(G.type===A[D]))
}if(b.detectDeviceModel){m(f(G.model),true)
}}if(b.detectScreen){G.screen={};
if(!!h&&!!h.mq){if(h.mq("only screen and (max-width: 240px)")){G.screen.size="veryVerySmall";
m("veryVerySmallScreen",true)
}else{if(h.mq("only screen and (max-width: 320px)")){G.screen.size="verySmall";
m("verySmallScreen",true)
}else{if(h.mq("only screen and (max-width: 480px)")){G.screen.size="small";
m("smallScreen",true)
}}}if(G.type===A[1]||G.type===A[2]){if(h.mq("only screen and (-moz-min-device-pixel-ratio: 1.3), only screen and (-o-min-device-pixel-ratio: 2.6/2), only screen and (-webkit-min-device-pixel-ratio: 1.3), only screen  and (min-device-pixel-ratio: 1.3), only screen and (min-resolution: 1.3dppx)")){G.screen.resolution="high";
m("highresolution",true)
}}}if(G.type===A[1]||G.type===A[2]){i.onresize=function(J){s(J)
};
s()
}else{G.orientation="landscape";
m(G.orientation,true)
}}if(b.detectOS){j.os={};
I=j.os;
if(G.model!==""){if(G.model==="ipad"||G.model==="iphone"||G.model==="ipod"){I.name="ios";
u(I,(t(/os\s([\d_]+)/)?RegExp.$1:"").replace(/_/g,"."))
}else{if(G.model==="android"){I.name="android";
u(I,(t(/android\s([\d\.]+)/)?RegExp.$1:""))
}else{if(G.model==="blackberry"){I.name="blackberry";
u(I,(t(/version\/([^\s]+)/)?RegExp.$1:""))
}else{if(G.model==="playbook"){I.name="blackberry";
u(I,(t(/os ([^\s]+)/)?RegExp.$1.replace(";",""):""))
}}}}}if(!I.name){if(g("win")||g("16bit")){I.name="windows";
if(g("windows nt 10")){u(I,"10")
}else{if(g("windows nt 6.3")){u(I,"8.1")
}else{if(g("windows nt 6.2")||t(/\(windows 8\)/)){u(I,"8")
}else{if(g("windows nt 6.1")){u(I,"7")
}else{if(g("windows nt 6.0")){u(I,"vista")
}else{if(g("windows nt 5.2")||g("windows nt 5.1")||g("windows xp")){u(I,"xp")
}else{if(g("windows nt 5.0")||g("windows 2000")){u(I,"2k")
}else{if(g("winnt")||g("windows nt")){u(I,"nt")
}else{if(g("win98")||g("windows 98")){u(I,"98")
}else{if(g("win95")||g("windows 95")){u(I,"95")
}}}}}}}}}}}else{if(g("mac")||g("darwin")){I.name="mac os";
if(g("68k")||g("68000")){u(I,"68k")
}else{if(g("ppc")||g("powerpc")){u(I,"ppc")
}else{if(g("os x")){u(I,(t(/os\sx\s([\d_]+)/)?RegExp.$1:"os x").replace(/_/g,"."))
}}}}else{if(g("webtv")){I.name="webtv"
}else{if(g("x11")||g("inux")){I.name="linux"
}else{if(g("sunos")){I.name="sun"
}else{if(g("irix")){I.name="irix"
}else{if(g("freebsd")){I.name="freebsd"
}else{if(g("bsd")){I.name="bsd"
}}}}}}}}}if(!!I.name){m(I.name,true);
if(!!I.major){c(I.name,I.major);
if(!!I.minor){c(I.name,I.major,I.minor)
}}}if(t(/\sx64|\sx86|\swin64|\swow64|\samd64/)){I.addressRegisterSize="64bit"
}else{I.addressRegisterSize="32bit"
}m(I.addressRegisterSize,true)
}if(b.detectBrowser){B=j.browser;
if(!t(/opera|webtv/)&&(t(/msie\s([\d\w\.]+)/)||g("trident"))){B.engine="trident";
B.name="ie";
if(!i.addEventListener&&o.documentMode&&o.documentMode===7){u(B,"8.compat")
}else{if(t(/trident.*rv[ :](\d+)\./)){u(B,RegExp.$1)
}else{u(B,(t(/trident\/4\.0/)?"8":RegExp.$1))
}}}else{if(g("firefox")){B.engine="gecko";
B.name="firefox";
u(B,(t(/firefox\/([\d\w\.]+)/)?RegExp.$1:""))
}else{if(g("gecko/")){B.engine="gecko"
}else{if(g("opera")){B.name="opera";
B.engine="presto";
u(B,(t(/version\/([\d\.]+)/)?RegExp.$1:(t(/opera(\s|\/)([\d\.]+)/)?RegExp.$2:"")))
}else{if(g("konqueror")){B.name="konqueror"
}else{if(g("edge")){B.engine="webkit";
B.name="edge";
u(B,(t(/edge\/([\d\.]+)/)?RegExp.$1:""))
}else{if(g("chrome")){B.engine="webkit";
B.name="chrome";
u(B,(t(/chrome\/([\d\.]+)/)?RegExp.$1:""))
}else{if(g("iron")){B.engine="webkit";
B.name="iron"
}else{if(g("crios")){B.name="chrome";
B.engine="webkit";
u(B,(t(/crios\/([\d\.]+)/)?RegExp.$1:""))
}else{if(g("applewebkit/")){B.name="safari";
B.engine="webkit";
u(B,(t(/version\/([\d\.]+)/)?RegExp.$1:""))
}else{if(g("mozilla/")){B.engine="gecko"
}}}}}}}}}}}if(!!B.name){m(B.name,true);
if(!!B.major){c(B.name,B.major);
if(!!B.minor){c(B.name,B.major,B.minor)
}}}m(B.engine,true);
B.language=a.userLanguage||a.language;
m(B.language,true)
}if(b.detectPlugins){B.plugins=[];
for(D=d.length-1;
D>=0;
D--){H=d[D];
F=false;
if(i.ActiveXObject){F=l(H.progIds)
}else{if(a.plugins){F=q(H.substrs)
}}if(F){B.plugins.push(H.name);
m(H.name,true)
}}if(a.javaEnabled()){B.plugins.push("java");
m("java",true)
}}}j.detect=function(B){return y(B)
};
j.init=function(){if(j!==e){j.browser={userAgent:(a.userAgent||a.vendor||i.opera).toLowerCase()};
j.detect()
}};
j.init();
return j
